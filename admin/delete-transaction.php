...
if(isset($_GET['txn'])){
    ...
    // Collect affected order IDs for audit
    $affected = [];
    $sel = mysqli_query($conn, "SELECT id FROM tbl_order WHERE transaction_number='$txn'");
    if ($sel) {
    ...
    if($res){
        ...
        @Audit::log('delete_transaction_orders', null, null, array('txn'=>$txn,'order_ids'=>$affected), null, array('count'=>count($affected)));
    } else {
        $_SESSION['delete'] = "<div class='error'>Failed to delete orders for transaction <b>$txn</b>.</div>";
        @Audit::log('delete_transaction_orders_failed', null, null, array('txn'=>$txn,'order_ids'=>$affected), null);
    }
} else {
