...
class Audit {
    ...
    public static function log($action, $entity=null, $entity_id=null, $old=null, $new=null, $meta=array()) {
        ...
        $old_json = $old ? json_encode($old, JSON_UNESCAPED_UNICODE) : null;
        $new_json = $new ? json_encode($new, JSON_UNESCAPED_UNICODE) : null;
        ...
        $sql = "INSERT INTO audit_logs (user_type, user_id, username, action, entity, entity_id, old_value, new_value, meta, ip, user_agent, url) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)";
        $stmt = mysqli_prepare($conn, $sql);
        if (!$stmt) return false;

        mysqli_stmt_bind_param($stmt, 'sissssssssss', $user_type, $user_id, $username, $action, $entity, $entity_id, $old_json, $new_json, $meta_json, $ip, $ua, $url);
