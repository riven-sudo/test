    ...
    if ($res && mysqli_num_rows($res) == 1) {
        ...
        if (password_verify($password, $row['password'])) {
            ...
            if ($row['status'] === 'Approved') {
                ...
                // Audit login success
                $new = array('customer_id' => $row['id'], 'username' => $row['username'], 'status' => $row['status']);
                @Audit::log('customer_login', 'tbl_customer', $row['id'], null, $new, array('success' => true));
            ...
            @Audit::log('customer_login_failed', null, null, null, array('username' => $username), array('reason' => 'invalid_password', 'success' => false));
            $_SESSION['login'] = "<div class='error'>Invalid password.</div>";
        }
    } else {
        // Audit failed login (user not found)
        @Audit::log('customer_login_failed', null, null, null, array('username' => $username), array('reason' => 'user_not_found', 'success' => false));
